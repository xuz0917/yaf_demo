...
class Db_Mysql {
	...
	function __construct($pConfig = 'default') {
		...
		$this->tablename = $tDB ['fix'] . $this->tablename;
	}
	...
	function __call($pMethod, $pArgs) {
		...
		if ('ff' == substr ( $pMethod, 0, 2 )) {
			return $this->where ( strtolower ( substr ( $pMethod, 2 ) ) . "='{$pArgs[0]}'" )->find ();
		}
	}
	...
	static function instance($pConfig = 'default') {
		if (empty ( self::$instance [$pConfig] )) {
			...
			self::$instance [$pConfig] = @new PDO ( $tDB ['dsn'], $tDB ['username'], $tDB ['password'], array (
					PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES ' . $tDB ['charset'] 
			) );
		}
	 ...
	 * 添加记录
	 */
	function add($datas, $pReplace = false) {
		if ($this->_filter ( $datas )) {
			if ($this->exec ( ($pReplace ? "REPLACE" : "INSERT") . " INTO `$this->tablename`(`" . implode ( '`,`', array_keys ( $datas ) ) . "`) VALUES ('" . implode ( "','", $datas ) . "')" )) {
	...
	function save($datas) {
		...
		if ($datas && ! empty ( $tOpt ['where'] )) {
			foreach ( $datas as $k1 => $v1 )
				$tSet [] = "`$k1`='$v1'";
			return $this->exec ( "UPDATE `" . $tOpt ['table'] . "` SET " . implode ( ',', $tSet ) . " WHERE " . $tOpt ['where'] );
		}
		return false;
	}
	...
	function del() {
		if ($tArgs = func_get_args ()) {
			...
			if (intval ( $tArgs [0] ) || count ( $tArgs ) > 1) {
				return $this->exec ( $tSql . $this->pk . ' IN(' . implode ( ',', array_map ( "intval", $tArgs ) ) . ')' );
			}
			// 传入删除条件
	...
	function find($pId = 0) {
		if (false === stripos ( $pId, 'SELECT' )) {
			$tOpt = $pId ? $this->_options ( array (
					'where' => $this->pk . '=' . abs ( $pId ) 
			) ) : $this->_options ();
	...
	function fOne($pField) {
		$this->field ( $pField );
		if (($tRow = $this->find ()) && isset ( $tRow [$pField] )) {
			return $tRow [$pField];
		}
		return false;
	}
	 ...
	 * 查找多条
	 */
	function select($pOpt = array()) {
		if (! is_array ( $pOpt )) {
			$pOpt = array (
					'where' => $this->pk . (strpos ( $pOpt, ',' ) ? ' IN(' . $pOpt . ')' : '=' . $pOpt) 
			);
		}
